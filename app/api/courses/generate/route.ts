...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    // Cloud-only: require at least one configured provider
    ...
    if (!hasCloudProviders) {
      ...
      console.error(`${logPrefix} 💡 Add at least one cloud API key to .env.local:`);
      console.error(`${logPrefix}    - GROQ_API_KEY`);
      console.error(`${logPrefix}    - GEMINI_API_KEY`);
      console.error(`${logPrefix}    - OPENROUTER_API_KEY`);
      console.error(`${logPrefix}    - ANTHROPIC_API_KEY`);
      console.error(`${logPrefix}    - OPENAI_API_KEY`);
      console.error(`${logPrefix}    - MISTRAL_API_KEY`);
      console.error(`${logPrefix}    - DEEPSEEK_API_KEY`);
      console.error(`${logPrefix}    - TOGETHER_API_KEY`);
      return NextResponse.json(
        {
          success: false,
          error: 'No AI provider available',
          message: 'Please configure at least one cloud API key',
